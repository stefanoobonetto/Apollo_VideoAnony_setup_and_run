...
LICENSE_PLATE_LABEL = "head"#"license plate"
...
def main(opt):
    ...
    head_classes = 0 #head_detector.model.names.index(HEAD_LABEL)
    print(head_classes)
    ...
    lp_classes = 0 #lp_detector.model.names.index(LICENSE_PLATE_LABEL)
    print(lp_classes)
    ...
    with tqdm(total=len(dataset)) as pbar:
        ...
        for path, im0s, vid_cap, s in dataset:
            for i in range(bs):
                ...
                imc = im0.copy()
                imc = head_detector.pre_process(imc)
                # Head detection
